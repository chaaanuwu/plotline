...
//                             <h2 className="text-2xl md:text-3xl font-bold">{user.user.firstName} {user.user.lastName}</h2>
//                         </div>
...
//                                     <span>
//                                         Joined {new Date(user.user.createdAt).toLocaleDateString(undefined, {
//                                             year: 'numeric',
//                                             month: 'long'
//                                         })}
//                                     </span>
...
export default function Profile() {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                setProfileData(response.data); // ✅ Set profileData here
    ...
    if (!profileData) return <p>Loading profile...</p>; // ✅ Show loading while fetching

    const isMyProfile = user?.user?._id === profileData?.user?._id; // ✅ user.user._id
                ...
                <div className="relative">
                    ...
                    <div className="px-4 -mt-16 md:-mt-20 flex flex-col md:flex-row md:items-end justify-between gap-4">
                        <div className="relative inline-block">
                            <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white overflow-hidden shadow-xl">
                                <img
                                    src={profileData.user.pfp || defaultPfp} // ✅ Use profileData here
                                    alt="Profile"
                                    className="w-full h-full object-cover"
                                    draggable="false"
                                />
                        ...
                        {/* Action Buttons */}
                        <div className="flex flex-wrap gap-2 items-start md:items-center">
                            { !isMyProfile && ( // ✅ Only show Follow if not my profile
                                <CustomButton
                                    onClick={() => console.log("Follow clicked")}
                                    className="bg-(--primary-color) text-white px-4 py-2 rounded-full hover:bg-(--interaction-color) hover:shadow-lg transition text-sm sm:text-base flex-1 sm:flex-auto min-w-30 text-center"
                                >
                    ...
                    {/* Pass profileData and isMyProfile to Tabs */}
